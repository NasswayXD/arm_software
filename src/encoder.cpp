...
static bool i2cReadReg(TwoWire& w, uint8_t addr, uint8_t reg, uint8_t& val, int retries=3){
  while (retries--){
    ...
    if (w.endTransmission(false) == 0) {
      if (w.requestFrom((int)addr, 1) == 1) { val = w.read(); return true; }
    }
    delay(2);
  }
  return false;
...
void enc_begin(AS5600Enc& e, TwoWire& w, int sda, int scl,
  ...
  if (do_begin) {
    // Force MASTER mode at the desired clock in one atomic call.
    // (Avoids a separate setClock() that triggers the “slave mode” check.)
    w.begin(sda, scl, clock_hz);
  }

  // IMPORTANT: remove any w.setClock(clock_hz); here
}
